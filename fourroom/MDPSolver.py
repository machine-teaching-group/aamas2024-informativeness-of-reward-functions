...
def valueIteration_H(env, reward, tol=1e-6):
    #print("inside valueIteration")
    # V = np.zeros((env.n_states))
    H = env.H
    Q_H_s_a = np.zeros((H, env.n_states, env.n_actions))
    Q_H_s_a[H-1, :, :] = copy.deepcopy(reward)
    # Q_H = np.zeros((env.n_states, env.n_actions))
    for h in range(H-2, -1, -1):
        V = np.max(Q_H_s_a[h+1, :, :], axis=1)
        for a in range(env.n_actions):
            Q_H_s_a[h, :, a] = reward[:, a] + env.gamma * env.T[:,:,a].dot(V)
...
def computeValueFunction_given_policy_st_linear_program(env, pi_s, reward):
    states_count = env.n_states
    actions_count = env.n_actions
    gamma = env.gamma
    R = reward
    ...
    T = get_T_pi(env, policy=pi_s)

    R_pi = get_R_pi(env, reward, pi_s)
...
def get_R_pi(env, reward, policy):
    R_pi = np.zeros((env.n_states))
    for s in range(env.n_states):
        for a in range(env.n_actions):
            R_pi[s] += reward[s, a] * policy[s, a]

    return R_pi
# enddef

def computemu_s_a_given_policy(env, policy):
    if len(policy.shape) == 1:
...
def compute_mu_s_a_given_policy_linear_program(env, policy):
    ...
    for a in range(env.n_actions):
        mu_s_a[:, a] = changed_policy[:, a] * mu_s[:]
    return mu_s_a, mu_s
...
def compute_Q_V_Function_given_policy(env, policy, reward, tol=1e-6):
    ...
    return Q_pi, V_pi
def compute_Q_V_Function_given_policy_st(env, policy, reward, tol=1e-6):
    V_pi = computeValueFunction_given_policy_st_linear_program(env, policy, reward)
    Q_pi = np.zeros((env.n_states, env.n_actions))
...
def computeFeature_mu_SA_bellmann(env, policy, tol=1e-6):
    mu_s_a = np.zeros((env.n_states, env.n_actions))
    SVF = computeFeatureSVF_bellmann(env, policy, tol)
    mu_s = (1-env.gamma) * SVF
    for a in range(env.n_actions):
        mu_s_a[:, a] = mu_s[:] * policy[:, a]
