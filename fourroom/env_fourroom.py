...
class Environment(gym.Env):
    def __init__(self, env_args):
        ...
        self.n2_subgoal_x_coord = 1
        self.n2_subgoal_y_coord = 0
        ...
        # self.state_action_feature_matrix = self.get_state_action_feature_matrix_one_hot()
    ...
    def get_reward(self):
        ...
        # reward for n2 subgoal is given only for UP action
            state = self.get_state_from_coord(self.n2_subgoal_x_coord, self.n2_subgoal_y_coord)
    ...
    def get_goal_states(self):
        ...
        return [self.gridsizefull * self.gridsizefull - 1]
    #enddef
    ...
    def get_state_action_feature_matrix(self):
        ...
        for s in range(self.n_states):
            for a in range(self.n_actions):
                state_action_feature_vector = self.get_state_action_feature_vector_square(s, a)
                state_action_feature_matrix.append(state_action_feature_vector)

        return state_action_feature_matrix
    #enddef

    def get_state_action_feature_matrix_one_hot(self):
        state_action_feature_matrix = []
        for s in range(self.n_states):
            for a in range(self.n_actions):
                state_action_feature_vector = self.get_state_action_feature_vector_one_hot(s, a)
                state_action_feature_matrix.append(state_action_feature_vector)

        return state_action_feature_matrix
    #enddef

    def get_state_action_feature_vector_one_hot(self, state, action):
        state_action_feature_vector = np.zeros(self.n_states * self.n_actions)
        state_action_feature_vector[state * self.n_actions + action] = 1.0
        return state_action_feature_vector
    #endef

    def get_state_action_feature_vector_square(self, state, action):
        states_for_feature_vector = [
                                        #room 1
        ...
        feature_dim = len(states_for_feature_vector) + 1 # for goal state
        state_feature_vector = np.zeros(feature_dim)
        state_action_feature_vector = np.zeros(feature_dim * self.n_actions)

        # check states in square
        for i, itm in enumerate(states_for_feature_vector):
            if state in itm:
                state_feature_vector[i] = 1.0
        # check for goal state
        if state in self.goal_state:
            state_feature_vector[feature_dim - 1] = 1.0
        # add action flag
        state_action_feature_vector[feature_dim*action:
                                    feature_dim*action+feature_dim] = state_feature_vector
        # input()
