...
class Agent:
    def __init__(self, env, args, exp_iter, use_pool=None):
        ...
        if use_pool:
            self.actor_theta = self.input_theta_from_file(exp_iter%5)
        else:
            self.actor_theta = np.zeros((env.n_states, env.n_actions))
        # self.actor_theta = np.random.uniform(low=-1, high=1, size=(env.n_states, env.n_actions))
    ...
    def input_theta_from_file(self, exp_iter):
        path = "{}/policy_{}.txt".format(self.args["learner_population"], exp_iter)
