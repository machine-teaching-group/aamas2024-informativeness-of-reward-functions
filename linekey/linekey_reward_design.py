...
def reward_design_model_based_invariant(env_0,  pi_d_T, pi_s_T, R_max,
    ...
    # reward \in R^|S|*|A|
    # R = cp.hstack(get_reward_given_w_symmetric(env_0))
    R = cp.hstack(get_reward_given_w_no_symmetric(env_0))
...
def calculate_ADV_stoch(env, reward, pi, P_pi_star_toch, I):
    ...
    return Advantage
#enddef
def get_reward_given_w_no_symmetric(env):
    w = cp.Variable(env.n_actions * 5)
    R = []
    for state in range(env.n_states):
        if state == env.n_states - 1: # for terminal state
            for a in range(env.n_actions):
                R.append(0.0)
            continue
        key_location = False
        ...
        for action in range(env.n_actions):
            if env.haskey(state) == False: # No key Left branch of the tree
                if key_location == False:
                    if goal_location == False: # leaf 0
                        R.append(w[0*env.n_actions + action])
                    if goal_location == True: # leaf 1
                        R.append(w[1*env.n_actions + action])
                if key_location == True: # leaf 2
                    R.append(w[2 * env.n_actions + action])

            if env.haskey(state) == True: #right brunch of the tree
                if goal_location == False: # leaf 3
                    R.append( w[3*env.n_actions + action])
                if goal_location == True: # leaf 4
                    R.append(w[4*env.n_actions + action])
    # R_vec = []
    #
    # for s in range(env.n_states):
    #     for a in range(env.n_actions):
    #         R_vec.append(R[(s, a)])
    return R
def get_reward_given_w_symmetric(env):
    w = cp.Variable(env.n_actions * 6)
    R = []
    for state in range(env.n_states):
        if state == env.n_states - 1: # for terminal state
            for a in range(env.n_actions):
        ...
        key_location = False
        goal_location = False
        x_coord, key = env.states_in_zero_one_interval_with_key[state]
        # key location
        if x_coord in [env.states_in_zero_one_interval[2],
                       env.states_in_zero_one_interval[3]]:
            key_location = True
        #Goal location
        if x_coord in [env.states_in_zero_one_interval[-2],
                       env.states_in_zero_one_interval[-3]]:
        ...
        for action in range(env.n_actions):
            if key == 0: # No key Left branch of the tree
                if key_location == False:
                    if goal_location == False: # leaf 0
                        R.append(w[0*env.n_actions + action])
                    if goal_location == True: # leaf 1
            ...
            if key == 1: #right brunch of the tree
                if key_location == False:
                    if goal_location == False: # leaf 3
                        R.append( w[3*env.n_actions + action])
                    if goal_location == True: # leaf 4
                        R.append(w[4*env.n_actions + action])
                if key_location == True: # leaf 5
                    R.append(w[5 * env.n_actions + action])
    # R_vec = []
    #
    # for s in range(env.n_states):
    #     for a in range(env.n_actions):
